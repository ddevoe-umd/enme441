...
#    with machine.disable_irq() / enable_irq() to avoid preemption.
#
# 4. THE LOCK IS NOT FIFO. asyncio.Lock hands off to waiters in the order they
#    arrived. _thread's lock makes no such promise. Instead, whichever thread 
#    happens to run next wins. Here it does not matter, because the job queue
#    (a plain list, popped from the front) preserves command order, and
#    the lock only guards the queue itself. 
# =============================================================================
...
class Stepper:
    ...
    def __init__(self, pins, delay_us=2000):
        """
        pins     -- 4 GPIO pins wired to IN1..IN4 on the ULN2003.
    ...
    def _step(self, direction):
        ...
        This is NOT atomic with respect to the other motors:
        a preemptive switch can land between any two of the pin writes
        below. 
        """
